...
import ModalEmVerify from "../modals-status/employee/verify";
...
export default function MainUser() {
    ...
    const updateStatus = (status: any) => {
        ...
        fetch(process.env.NEXT_EMPLOYEE_UPDATE || '', {
            ...
            .then(data => {
                ...
                // Update item.userp_status based on API respons
                const updatedPackageDetails = packageDetails?.map(item => {
        ...
        return (
            <>
                <div className={`fixed inset-0 z-50 flex items-center justify-center ${isModalOpen ? "" : "hidden"}`}>
                    <div className="fixed inset-0 bg-black opacity-50"></div>
                    <div id="popup-modal" tabIndex={-1} className="relative p-4 w-full max-w-md max-h-full bg-white rounded-lg shadow dark:bg-gray-700">
                        <button type="button" className="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white">
                            <svg className="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                                <path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6" />
                            </svg>
                            <span className="sr-only">ปิด</span>
    ...
    useEffect(() => {
        const fetchPackageDetails = async () => {
            try {
                const response = await fetch(process.env.NEXT_EMPLOYEE_DATA || "");
                if (!response.ok) {
    ...
    const handleToggleModal = (pickup_id : string ,userv_id: string,user_id: string, userp_status: number) => {
        setIsModalOpen(!isModalOpen);
        console.log("pickup_id", pickup_id);
        console.log("userv_id", userv_id);
        console.log("user_id", user_id);
        setPickup_id(pickup_id);
                    ...
                    <div style={{ flex: 1, height: '100vh' }}>
                        {
                            statusShow === 0 ? (
                                <div className="flex flex-col items-center justify-center h-full">
                                    <div className="w-full md:max-w-screen-xl pt-5">
                                                    ...
                                                    <tbody>
                                                        {getCurrentPageData().map((item, index) => (
                                                            <tr key={item.userv_id} className="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
                                                                ...
                                                                <td className="px-6 py-4">
                                                                    <button
                                                                        onClick={() => handleToggleModal(item.pickup_id,item.userv_id,item.user_id, Number(item.userp_status))}
                                                                    ...
                                                                    >
                                                                        <span className={`inline-flex items-center ${Number(item.userp_status) === 0 ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900 dark:text-yellow-300' :
                                                                            Number(item.userp_status) === 1 ? 'bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-300' :
                                                                                Number(item.userp_status) === 2 ? 'bg-indigo-100 text-indigo-800 dark:bg-indigo-900 dark:text-indigo-300' :
                                                                                    Number(item.userp_status) === 3 ? 'bg-fuchsia-100 text-fuchsia-800 dark:bg-fuchsia-900 dark:text-fuchsia-300' :
                                                                            ...
                                                                            {Number(item.userp_status) === 0 ? 'รอเข้ารับพัสดุ' :
                                                                                Number(item.userp_status) === 1 ? 'เตรียมเข้ารับพัสดุ' :
                                                                                    Number(item.userp_status) === 2 ? 'แนบใบชำระเงิน' :
                                                                                        Number(item.userp_status) === 3 ? 'ตรวจสอบใบชำระเงิน' :
                                    ...
                                    {isModalOpen && (
                                        <>
                                            {status === 0 && ModalService(modalName, status)}
                                            {status === 2 && <ModalEmBill handleToggleModal={handleToggleModal} name={pickup_id} />}
                                            {status === 3 && <ModalEmVerify handleToggleModal={handleToggleModal} />}
                                            {/* {status === 2 && ModalImage(imageUrl)}
                                            {status === 3 && <ModalEmBill handleToggleModal={handleToggleModal} name={""} />}
                                            {status === 4 && <ModalEmCancel handleToggleModal={handleToggleModal} />} */}
                                        </>
                                    )}
                                </div>
                            ) : (
